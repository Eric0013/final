...
# 完美恢復一開始最穩定的單一網頁畫面 (修復前端 JS 對齊問題)
HTML_TEMPLATE = '''
<!DOCTYPE html>
<html lang="zh-TW">
<head>
    ...
    <script>
        document.getElementById('stockForm').addEventListener('submit', async (e) => {
            ...
            try {
                ...
                resultDiv.innerHTML = `<h4 class="mb-3 text-danger">錯誤</h4><p>連線異常或代碼輸入有誤，請稍後再試。</p>`;
            }
        });
    </script>
</body>
...
def get_stock_analysis_report(symbol):
    try:
        ...
        if df is None or df.empty:
            return f"❌ 找不到股票代碼 「{symbol}」 或 Yahoo 財經拒絕連線。若為台股請務必加上尾綴（如：2330.TW）。"

        if isinstance(df.columns, pd.MultiIndex):
            df.columns = df.columns.get_level_values(0)
        
        if len(df) < 200:
            return f"❌ 股票代碼 {symbol} 歷史資料不足 (需至少200個交易日以計算長天期均線)"
...
def analyze():
    ...
    # 安全修正：一律回傳 200 成功 JSON，絕不用 500 逼死前端
